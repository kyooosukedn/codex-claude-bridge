...
export async function acquireSessionLock({
  ...
  // A dead bridge PID does not prove its spawned transport process is dead.
  // Reclaiming an "injecting" generation could overlap an orphaned ccmux/tmux
  // child and corrupt the prompt stream. Leave this lock fenced until an
  // operator inspects the pane and removes the lock deliberately.
  if (validated.phase === "injecting") {
    return {
      acquired: false,
      reason: "uncertain-dead-owner",
      lockPath,
      ownerToken,
      commandId: commandIdResolved,
      recovered: false,
      priorPhase: validated.phase,
      heldBy,
      states,
    };
  }
...
export async function coordinateInjection({
  ...
  const recordJournal = async (state, attempt, fields = {}) => {
    if (!journal) return;
  ...
  for (let attempt = 1; attempt <= attemptsLimit; attempt += 1) {
    try {
      await recordJournal("queued", attempt, { safeToRetry: true });
    } catch (error) {
      return {
        commandId: resolvedCommandId,
        commandClass,
    ...
    if (!acquired.acquired) {
      try {
        await recordJournal("released", attempt, {
          ack: "busy",
          safeToRetry: true,
        ...
        return {
          ...
          reason: "journal-error",
          payload: null,
          states: [...(acquired.states || ["queued"]), "released"],
          error: error.message,
        };
      }
      return {
    ...
    const states = [...(acquired.states || ["queued"])];
    if (states.at(-1) !== "pre-write") states.push("pre-write");
    try {
      await recordJournal("pre-write", attempt, { safeToRetry: true });
    } catch (error) {
    ...
    try {
      ...
      try {
        await recordJournal("released", attempt, {
          ack: "not-injected",
          // A baseline-capture failure happens strictly before any transport
          // write (inject is never called), so nothing reached the session.
          // It is therefore ALWAYS safe for a caller to retry — even when the
          // automatic attempt budget is exhausted. Tying this to
          // `attempt < attemptsLimit` made a proven pre-write failure record
          // safeToRetry=false once the budget ran out, contradicting the
          // documented not-injected-is-safe-to-retry contract. We do not auto-
          // retry here: mode-changing inputs run with attemptsLimit=1, and a
          // baseline failure usually means the session/capture is unhealthy.
          safeToRetry: true,
    ...
    try {
      await recordJournal("injecting", attempt, { safeToRetry: false });
    } catch (error) {
      await releaseSessionLock({
        lockPath: acquired.lockPath,
        ownerToken: acquired.ownerToken,
    ...
    if (!observation || observation.observed !== true) {
      ...
      let reason = observation?.reason || "not-observed";
      let journalError;
      try {
        await recordJournal("released", attempt, {
          ack: "uncertain",
      ...
      return {
        ...
        payload,
        states: [...states, "released"],
        ...(journalError ? { error: journalError.message } : {}),
      };
    }

    const injectedAt = asIso(now());
    try {
      await recordJournal("acknowledged", attempt, {
        ack: "injected",
        safeToRetry: false,
    ...
    if (!acknowledged.updated || !released.released) {
      let reason = !acknowledged.updated ? "ack-update-failed" : "release-failed";
      let journalError;
      try {
        await recordJournal("interrupted", attempt, {
          ack: "uncertain",
          safeToRetry: false,
          reason,
        });
